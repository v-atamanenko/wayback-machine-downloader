...
class WaybackMachineDownloader
  ...
  def get_all_snapshots_to_consider
    # Note: Passing a page index parameter allows us to get more snapshots,
    ...
    loop do
      begin
        ...
        # Handle the error
        puts "get_all_snapshots_to_consider / http.start() error: #{e.message}"
        ...
        if current_retry <= max_retries
          ...
          puts "Retrying in 3*current_retry seconds..."
          sleep(3*current_retry)
        else
          puts "Max retries reached. Exiting..."
          # You might want to raise the error or handle it differently here
          raise e
        end
      end
    end
    ...
    @threads_count.times do
      ...
      loop do
        begin
          ...
          # Handle the error
          puts "download_files / http.start() error: #{e.message}"
